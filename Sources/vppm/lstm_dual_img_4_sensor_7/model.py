"""VPPM-LSTM-Dual-Img-4-Sensor-7 모델.
...
3-분기 LSTM:
  ...
  feats14 (G3+G1+G4) ⊕ embed_v0 ⊕ embed_v1 ⊕ embed_s ──> (B, 29) ──MLP──> (B, 1)
...
class _SensorLSTMBranch(nn.Module):
    """7-channel sensor 시퀀스 → LSTM → proj(d_embed)."""
    ...
    def forward(self, sensors: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        # sensors: (B, T, n_channels) float32, lengths: (B,) int64 (cpu)
        lengths_cpu = lengths.detach().cpu()
        packed = pack_padded_sequence(
            sensors, lengths_cpu, batch_first=True, enforce_sorted=False,
        )
...
class VPPM_LSTM_Dual_Img_4_Sensor_7(nn.Module):
    ...
    MLP 입력: 14 (G3+G1+G4) + d_embed_v0(4) + d_embed_v1(4) + d_embed_s(7) = 29.
    """
    ...
    def __init__(self,
        ...
        # 결합 MLP — 입력 = 14 (G3+G1+G4) + d_embed_v0 + d_embed_v1 + d_embed_s
        n_baseline_kept = n_baseline_feats - n_sensor_feats_dropped   # 21 - 7 = 14
    ...
    def _init_mlp_weights(self):
        """MLP 와 세 proj 만 baseline 과 동일하게 N(0, 0.1) 초기화."""
    ...
    def encode_all(self, stacks_v0: torch.Tensor, stacks_v1: torch.Tensor,
        ...
        return embed_v0, embed_v1, embed_s

    def forward(self,
                feats14: torch.Tensor,
                stacks_v0: torch.Tensor, stacks_v1: torch.Tensor,
                sensors: torch.Tensor,
                lengths: torch.Tensor) -> torch.Tensor:
        # feats14:    (B, 14) — G2 제거된 baseline (G3 3 + G1 8 + G4 3)
        # stacks_v*:  (B, T, 8, 8)
        # sensors:    (B, T, 7)
        # lengths:    (B,) int64 — 세 분기 공통
        embed_v0, embed_v1, embed_s = self.encode_all(stacks_v0, stacks_v1, sensors, lengths)

...
| **사용 피처 수** | 21 → **14** |
| **출력 디렉터리** | `Sources/pipeline_outputs/experiments/baseline_ablation/E2_no_sensor/` |
...
```bash
./venv/bin/python -m Sources.vppm.baseline_ablation.run --experiment E2
./venv/bin/python -m Sources.vppm.baseline_ablation.run --experiment E2 --quick
```

**도커:**

```bash
cd docker/baseline_ablation/sensor
./run.sh              # 기본 (GPU 1)
./run.sh --quick
```
...
```
Sources/pipeline_outputs/experiments/baseline_ablation/E2_no_sensor/
...
> ```bash
> ./venv/bin/python -m Sources.vppm.baseline_ablation.analyze_per_build --experiment E2
> # → Sources/pipeline_outputs/experiments/baseline_ablation/E2_no_sensor/per_build_analysis.md
> ```

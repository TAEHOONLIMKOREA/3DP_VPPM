...
산출물 구조 (baseline `pipeline_outputs/{models,results,features}` 레이아웃을 각 실험 폴더 내부에서 그대로 재사용):
    Sources/pipeline_outputs/experiments/baseline_ablation/<exp_id>_no_<group>/
...
Usage:
    ./venv/bin/python -m Sources.vppm.baseline_ablation.run --experiment E1
    ./venv/bin/python -m Sources.vppm.baseline_ablation.run --all
    ./venv/bin/python -m Sources.vppm.baseline_ablation.run --experiment E3 --quick  # smoke test
"""
...
# Sources/vppm/baseline_ablation/run.py → 프로젝트 루트는 parents[3]
...
EXPERIMENTS = {
    ...
    "E12": ("dscnn_excessive_melting",  "No-ExcessiveMelt — 과다 용융 제거 (B1.2 Keyhole 핵심 후보)"),
    "E23": ("dscnn_defects_all",        "No-DefectsAll — DSCNN 결함 6채널 일괄 제거 (normal 2개만 남김)"),
    "E24": ("dscnn_normal",             "No-DSCNNNormal — DSCNN normal 2채널 제거 (defect 6개만 남김)"),
    # --- 센서 서브 채널 ablation (PLAN_sensor_subablation.md) ---

...
| **제거 피처 idx** | 18, 19, 20 |
| **사용 피처 수** | 21 → **18** |
| **출력 디렉터리** | `Sources/pipeline_outputs/experiments/baseline_ablation/E4_no_scan/` |
...
```bash
./venv/bin/python -m Sources.vppm.baseline_ablation.run --experiment E4
./venv/bin/python -m Sources.vppm.baseline_ablation.run --experiment E4 --quick
```

**도커:**

```bash
cd docker/baseline_ablation/scan
./run.sh              # 기본 (GPU 3)
./run.sh --quick
```
...
```
Sources/pipeline_outputs/experiments/baseline_ablation/E4_no_scan/
...
> Phase 5 (전체 ablation 재실행) 완료 후 [FULL_REPORT.md](../../pipeline_outputs/experiments/baseline_ablation/FULL_REPORT.md) 와
> [summary.md](../../pipeline_outputs/experiments/baseline_ablation/summary.md) 에서 v2 결과 확인.

### 4.1 빌드별 잔차 분해

```bash
./venv/bin/python -m Sources.vppm.baseline_ablation.analyze_per_build --experiment E4
# → Sources/pipeline_outputs/experiments/baseline_ablation/E4_no_scan/per_build_analysis.md
```
...
- 종합 보고서: [FULL_REPORT.md](../../pipeline_outputs/experiments/baseline_ablation/FULL_REPORT.md)
- 스캔 서브 채널 분해: [docker/baseline_ablation/scan_sub/README.md](../../../docker/baseline_ablation/scan_sub/README.md)

"""Phase S4 — VPPM-LSTM-Dual-Img-16-DSCNN-8-CAD-8-Scan-8-1DCNN-Sensor-4 평가.

`lstm_dual_img_4_sensor_7_dscnn_8/evaluate.py` 와 동일 골격.
차이는 모델 클래스 / 파일 prefix / cad_patch + scan_patch 입력 추가 / static 2-feat.
"""
...
def _evaluate_fold(
    model: VPPM_LSTM_Dual_Img_16_DSCNN_8_CAD_8_Scan_8_1DCNN_Sensor_4,
...
def evaluate_all(
    dataset: dict,
    models_dir: Path = config.LSTM_FULL86_MODELS_DIR,
    device: str = "cpu",
) -> dict:
    ...
    for prop in config.TARGET_PROPERTIES:
        ...
        print(f"\n{short}:")
        print(f"  VPPM-LSTM-Full-86 RMSE: {mean_rmse:.2f} ± {std_rmse:.2f}")

"""Phase D3 (dual_4) — VPPM-LSTM-Dual-4 평가

`lstm_dual/evaluate.py` 와 동일 골격. 차이는 모델 클래스 / 파일 prefix / 기본 디렉토리.
"""
...
def _evaluate_fold(model: VPPM_LSTM_Dual_4,
                   feats: np.ndarray,
...
def evaluate_all(dataset: dict,
                 models_dir: Path = config.LSTM_DUAL_4_MODELS_DIR,
                 device: str = "cpu") -> dict:
    ...
    for prop in config.TARGET_PROPERTIES:
        ...
        for fold, (_, val_mask) in enumerate(splits):
            mp = Path(models_dir) / f"vppm_lstm_dual_4_{short}_fold{fold}.pt"
            if not mp.exists():
                print(f"  warn: {mp} 없음, skip")
                continue
            model = VPPM_LSTM_Dual_4()
        ...
        print(f"\n{short}:")
        print(f"  VPPM-LSTM-Dual-4 RMSE: {mean_rmse:.2f} ± {std_rmse:.2f}")

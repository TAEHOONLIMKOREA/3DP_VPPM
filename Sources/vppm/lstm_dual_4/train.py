"""Phase D2 (dual_4) — VPPM-LSTM-Dual-4 학습 (4 properties × 5 folds = 20 모델)

`lstm_dual/train.py` 와 동일 골격. 차이는 (1) 모델 클래스 `VPPM_LSTM_Dual_4`,
(2) 저장 파일명 `vppm_lstm_dual_4_{short}_fold{k}.pt`,
(3) 기본 출력 디렉토리 `LSTM_DUAL_4_MODELS_DIR`.
"""
...
from .model import VPPM_LSTM_Dual_4
...
def train_single_fold(
    ...
    model = VPPM_LSTM_Dual_4().to(device)
...
def train_all(dataset: dict,
              output_dir: Path = config.LSTM_DUAL_4_MODELS_DIR,
              device: str = "cpu") -> dict:
    """4 properties × 5 fold 학습. 모델 저장 → output_dir/vppm_lstm_dual_4_{short}_fold{k}.pt."""
    ...
    for prop in config.TARGET_PROPERTIES:
        ...
        print(f"\n{'='*60}\nTraining VPPM-LSTM-Dual-4 for {short}\n{'='*60}")
        fold_results = []
        for fold, (train_mask, val_mask) in enumerate(splits):
            ...
            model_path = output_dir / f"vppm_lstm_dual_4_{short}_fold{fold}.pt"

"""Phase S4 — VPPM-LSTM-Dual-Img-4-Sensor-7-DSCNN-8 평가.

`lstm_dual_img_4_sensor_7/evaluate.py` 와 동일 골격.
차이는 모델 클래스 / 파일 prefix / dscnn 입력 추가.
"""
...
def _evaluate_fold(model: VPPM_LSTM_Dual_Img_4_Sensor_7_DSCNN_8,
                   feats: np.ndarray,
    ...
    with torch.no_grad():
        for i0 in range(0, N, batch_size):
            ...
            out = model(f, s0, s1, sn, dn, l).cpu().numpy().flatten()
...
def evaluate_all(dataset: dict,
                 models_dir: Path = config.LSTM_DUAL_IMG_4_SENSOR_7_DSCNN_8_MODELS_DIR,
                 device: str = "cpu") -> dict:
    ...
    for prop in config.TARGET_PROPERTIES:
        ...
        for fold, (_, val_mask) in enumerate(splits):
            ...
            if not mp.exists():
                ...
                continue
            model = VPPM_LSTM_Dual_Img_4_Sensor_7_DSCNN_8()
        ...
        print(f"\n{short}:")
        print(f"  VPPM-LSTM-Dual-Img-4-Sensor-7-DSCNN-8 RMSE: {mean_rmse:.2f} ± {std_rmse:.2f}")

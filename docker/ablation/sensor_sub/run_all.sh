#!/usr/bin/env bash
# 센서 서브 9개 실험(E14~E22)을 4-GPU 에 배치 스케줄로 병렬 실행.
#
# 호스트 GPU 구성: 0,1,2,3 (4장)
#   Batch 1: E14→GPU0  E15→GPU1  E16→GPU2  E17→GPU3
#   Batch 2: E18→GPU0  E19→GPU1  E20→GPU2  E21→GPU3
#   Batch 3: E22→GPU0
#
...
#
# Usage:
#   ./run_all.sh           # 전체 학습 (GPU 4장 병렬, ~30~45분)
#   ./run_all.sh --quick   # smoke test (각 실험 2분 이내)
set -euo pipefail
cd "$(dirname "$0")"
...
EXPERIMENT_ID=E14 NVIDIA_VISIBLE_DEVICES=0 "${DC[@]}" build

LOG_DIR="/tmp/sensor_sub_logs_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
echo "[run_all] logs: $LOG_DIR"

# ── 2) 배치 실행 헬퍼 ────────────────────────────────────
# $1: 배치 라벨 (표시용), $2+: "EXP:GPU" 쌍 목록
run_batch() {
  local label="$1"; shift
  echo ""
  echo "[run_all] === $label ==="
...
overall_fail=0
run_batch "Batch 1/3 — E14~E17" \
  E14:0 E15:1 E16:2 E17:3 || overall_fail=1

run_batch "Batch 2/3 — E18~E21" \
  E18:0 E19:1 E20:2 E21:3 || overall_fail=1

run_batch "Batch 3/3 — E22 (단독)" \
  E22:0 || overall_fail=1
...
if [ -x ./venv/bin/python ]; then
  ...
  echo "    (cd docker/ablation/sensor_sub && \\"
  echo "      EXPERIMENT_ID=E14 NVIDIA_VISIBLE_DEVICES=0 sudo -E docker compose run --rm \\"
  echo "      ablation-sensor-sub /workspace/venv/bin/python -m Sources.vppm.ablation.run --rebuild-summary)"
fi
